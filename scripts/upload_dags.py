# see: https://cloud.google.com/composer/docs/composer-2/dag-cicd-github?hl=ja#dags-utility
...
def _create_dags_list(dags_directory: str) -> tuple[str, list[str]]:
    ...
    files_to_ignore = ignore_patterns("__init__.py", "test_*.py", "conftest.py")
    ...
    dags = glob.glob(f"{temp_dir}/**/*.py", recursive=True) + glob.glob(
        f"{temp_dir}/**/*.sql", recursive=True
    )
...
def upload_dags_to_composer(
    ...
    Args:
        ...
        name_replacement (str, optional): the name of the "dags/" subdirectory
        that will be used when constructing the temporary directory path name Defaults to "dags/".
    """
